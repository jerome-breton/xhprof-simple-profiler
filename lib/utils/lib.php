...
//
// This file contains various profiler library (utility) functions.
// Do not add any display specific code here.
//

function profiler_error($message) {
  error_log($message);
}

/*
 * The list of possible metrics collected as part of profiler that
 * require inclusive/exclusive handling while reporting.
 *
 * @author Kannan
 */
function profiler_get_possible_metrics() {
 ...
 * @author Kannan
 */
function init_metrics($profiler_data, $rep_symbol, $sort, $diff_report = false) {
  global $stats;
  global $pc_stats;
  global $metrics;
  global $diff_mode;
  ...
  if (!isset($profiler_data["main()"]["wt"])) {
  ...
  $possible_metrics = profiler_get_possible_metrics($profiler_data);
  foreach ($possible_metrics as $metric => $desc) {
    if (isset($profiler_data["main()"][$metric])) {
      $metrics[] = $metric;
...
/*
 * Get the list of metrics present in $profiler_data as an array.
 *
 * @author Kannan
 */
function profiler_get_metrics($profiler_data) {

  // get list of valid metrics
  $possible_metrics = profiler_get_possible_metrics();
 ...
 * @author Kannan
 */
function profiler_parse_parent_child($parent_child) {
 ...
 * @author Kannan
 */
function profiler_build_parent_child_key($parent, $child) {
  if ($parent) {
    return $parent . "==>" . $child;
  } else {
    return $child;
...
/**
 * Checks if profiler raw data appears to be valid and not corrupted.
 *
 *  @param   int    $run_id        Run id of run to be pruned.
 *                                 [Used only for reporting errors.]
 *  @param   array  $raw_data      profiler raw data to be pruned
 *                                 & validated.
 *
 ...
 *  @author Kannan
 */
function profiler_valid_run($run_id, $raw_data) {

  $main_info = $raw_data["main()"];
  if (empty($main_info)) {
    profiler_error("profiler: main() missing in raw data for Run ID: $run_id");
    return false;
  }
  ...
  if (isset($main_info["wt"])) {
    ...
    profiler_error("profiler: Wall Time information missing from Run ID: $run_id");
    return false;
  }
  ...
  foreach ($raw_data as $info) {
    ...
    // basic sanity checks...
    if ($val < 0) {
      profiler_error("profiler: $metric should not be negative: Run ID $run_id"
                   . serialize($info));
      return false;
    }
    if ($val > (86400000000)) {
      profiler_error("profiler: $metric > 1 day found in Run ID: $run_id "
                   . serialize($info));
      return false;
    }
  }
...
/**
 * Return a trimmed version of the profiler raw data. Note that the raw
 ...
 *
 * @param  array  profiler raw data
 * @param  array  array of function names
 *
 * @return array  Trimmed profiler Report
 *
 * @author Kannan
 */
function profiler_trim_run($raw_data, $functions_to_keep) {
  ...
  foreach ($raw_data as $parent_child => $info) {
    list($parent, $child) = profiler_parse_parent_child($parent_child);
...
/**
 * Takes raw profiler data that was aggregated over "$num_runs" number
 * of runs averages/normalizes the data. Essentially the various metrics
 * collected are divided by $num_runs.
 *
 * @author Kannan
 */
function profiler_normalize_metrics($raw_data, $num_runs) {
  ...
  if (isset($raw_data["==>main()"]) && isset($raw_data["main()"])) {
    profiler_error("profiler Error: both ==>main() and main() set in raw data...");
  }
 ...
 *
 *     profiler_aggregate_runs(array(5, 6, 7), array(2, 4, 1));
 *
 * The above will return raw data for the runs aggregated
 * in 2:4:1 ratio.
 *
 *  @param object  $profiler_runs_impl  An object that implements
 *                                    the iProfilerRuns interface
 *  @param  array  $runs            run ids of the profiler runs..
 ...
 *  @author Kannan
 */
function profiler_aggregate_runs($profiler_runs_impl, $runs,
                               $wts, $source="phprof",
  ...
  foreach ($runs as $idx => $run_id) {
    ...
    $raw_data = $profiler_runs_impl->get_run($run_id, $source, $description);
    ...
    if (!profiler_valid_run($run_id, $raw_data)) {
      $bad_runs[] = $run_id;
      continue;
    }
    ...
    if ($use_script_name) {
      ...
      if ($page) {
        ...
        $raw_data[profiler_build_parent_child_key("main()",
                                                "__script::$page")]
          = $fake_edge;
      } else {
        $use_script_name = false;
      ...
      if ($use_script_name) {
        ...
        if (substr($parent_child, 0, 9) == "main()==>") {
          ...
          if (substr($child, 0, 10) != "__script::") {
            $parent_child = profiler_build_parent_child_key("__script::$page",
                                                          $child);
          }
        }
      }
  ...
  $data['raw'] = profiler_normalize_metrics($raw_data_total,
                                          $normalization_count);
 ...
 *
 * @param  array $raw_data          profiler format raw profiler data.
 ...
 */
function profiler_compute_flat_info($raw_data, &$overall_totals) {

  global $display_calls;

  $metrics = profiler_get_metrics($raw_data);
  ...
  $symbol_tab = profiler_compute_inclusive_times($raw_data);
 ...
 * @author Kannan
 */
function profiler_compute_diff($profiler_data1, $profiler_data2) {
  global $display_calls;

  // use the second run to decide what metrics we will do the diff on
  $metrics = profiler_get_metrics($profiler_data2);

  $profiler_delta = $profiler_data2;

  foreach ($profiler_data1 as $parent_child => $info) {

    if (!isset($profiler_delta[$parent_child])) {

      // this pc combination was not present in run1;
      // initialize all values to zero.
      if ($display_calls) {
        $profiler_delta[$parent_child] = array("ct" => 0);
      } else {
        $profiler_delta[$parent_child] = array();
      }
      foreach ($metrics as $metric) {
        $profiler_delta[$parent_child][$metric] = 0;
      }
    }

    if ($display_calls) {
      $profiler_delta[$parent_child]["ct"] -= $info["ct"];
    }

    foreach ($metrics as $metric) {
      $profiler_delta[$parent_child][$metric] -= $info[$metric];
    }
  }

  return $profiler_delta;
}


/**
 * Compute inclusive metrics for function. This code was factored out
 * of profiler_compute_flat_info().
 *
 ...
 * @author Kannan
 */
function profiler_compute_inclusive_times($raw_data) {
  global $display_calls;
  ...
  foreach ($raw_data as $parent_child => $info) {
    ...
    if ($parent == $child) {
      /*
       * profiler PHP extension should never trigger this situation any more.
       * Recursion is handled in the profiler PHP extension by giving nested
 ...
 * "__pruned__()".]
 *
 *  @param   array  $raw_data      profiler raw data to be pruned & validated.
 ...
 *  @author Kannan
 */
function profiler_prune_run($raw_data, $prune_percent) {

  $main_info = $raw_data["main()"];
  if (empty($main_info)) {
    profiler_error("profiler: main() missing in raw data");
    return false;
  }
  ...
  if (isset($main_info["wt"])) {
    ...
    profiler_error("profiler: for main() we must have either wt "
  ...
  $flat_info = profiler_compute_inclusive_times($raw_data);
  ...
  foreach ($raw_data as $parent_child => $info) {
    ...
    if ($flat_info[$child][$prune_metric] < $prune_threshold) {
      ...
      $pruned_edge = profiler_build_parent_child_key("__pruned__()", $child);
 ...
 * @author Kannan
 */
function profiler_array_set($arr, $k, $v) {
  $arr[$k] = $v;
  return $arr;
}
 ...
 * @author Kannan
 */
function profiler_array_unset($arr, $k) {
  unset($arr[$k]);
  return $arr;
}
 ...
 */
define('PROFILER_STRING_PARAM', 1);
define('PROFILER_UINT_PARAM',   2);
define('PROFILER_FLOAT_PARAM',  3);
define('PROFILER_BOOL_PARAM',   4);


/**
 * Internal helper function used by various
 * profiler_get_param* flavors for various
 * types of parameters.
 *
 ...
 * @author Kannan
 */
function profiler_get_param_helper($param) {
  $val = null;
  if (isset($_GET[$param]))
    $val = $_GET[$param];
 ...
 * @author Kannan
 */
function profiler_get_string_param($param, $default = '') {
  $val = profiler_get_param_helper($param);
 ...
 * @author Kannan
 */
function profiler_get_uint_param($param, $default = 0) {
  $val = profiler_get_param_helper($param);
  ...
  profiler_error("$param is $val. It must be an unsigned integer.");
  return null;
}
 ...
 * @author Kannan
 */
function profiler_get_float_param($param, $default = 0) {
  $val = profiler_get_param_helper($param);
  ...
  profiler_error("$param is $val. It must be a float.");
  return null;
}
 ...
 * @author Kannan
 */
function profiler_get_bool_param($param, $default = false) {
  $val = profiler_get_param_helper($param);
  ...
  case 'no':
    ...
    break;
  default:
    profiler_error("$param is $val. It must be a valid boolean string.");
    return null;
  }
 ...
 * @author Kannan
 */
function profiler_param_init($params) {
  /* Create variables specified in $params keys, init defaults */
  foreach ($params as $k => $v) {
    switch ($v[0]) {
    case PROFILER_STRING_PARAM:
      $p = profiler_get_string_param($k, $v[1]);
      break;
    case PROFILER_UINT_PARAM:
      $p = profiler_get_uint_param($k, $v[1]);
      break;
    case PROFILER_FLOAT_PARAM:
      $p = profiler_get_float_param($k, $v[1]);
      break;
    case PROFILER_BOOL_PARAM:
      $p = profiler_get_bool_param($k, $v[1]);
      break;
    default:
 ...
 * specified profiler run. This is used for the type ahead function
 * selector.
 *
 * @author Kannan
 */
function profiler_get_matching_functions($q, $profiler_data) {

  $matches = array();

  foreach ($profiler_data as $parent_child => $info) {
    list($parent, $child) = profiler_parse_parent_child($parent_child);
