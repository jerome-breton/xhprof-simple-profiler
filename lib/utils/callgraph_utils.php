...
/*
 * This file contains callgraph image generation related profiler utility
 * functions
 *
 */

// Supported output format
$profiler_legal_image_types = array(
    "jpg" => 1,
    "gif" => 1,
    "png" => 1,
 ...
 *
 */
function profiler_http_header($name, $value) {

  if (!$name) {
    profiler_error('http_header usage');
    return null;
  }

  if (!is_string($value)) {
    profiler_error('http_header value not a string');
  }
 ...
 * @author cjiang
 */
function profiler_generate_mime_header($type, $length) {
  switch ($type) {
    case 'jpg':
      $mime = 'image/jpeg';
      break;
  ...
  if ($mime) {
    profiler_http_header('Content-type', $mime);
    profiler_http_header('Content-length', (string)$length);
  }
}
 ...
 * $profiler_legal_image_types.
 * @returns, binary content of the generated image on success. empty string on
 *           failure.
 *
 * @author cjiang
 */
function profiler_generate_image_by_dot($dot_script, $type) {
 ...
 */
function profiler_get_children_table($raw_data) {
 ...
 * @author cjiang
 */
function profiler_generate_dot_script($raw_data, $threshold, $source, $page,
  ...
  $sym_table = profiler_compute_flat_info($raw_data, $totals);

  if ($critical_path) {
    $children_table = profiler_get_children_table($raw_data);
    $node = "main()";
    $path = array();
    $path_edges = array();
    $visited = array();
    while ($node) {
      ...
      if (isset($children_table[$node])) {
        ...
        foreach ($children_table[$node] as $child) {
          ...
          if ($max_child === null ||
            abs($raw_data[profiler_build_parent_child_key($node,
                                                        $child)]["wt"]) >
            abs($raw_data[profiler_build_parent_child_key($node,
                                                        $max_child)]["wt"])) {
            $max_child = $child;
        ...
        if ($max_child !== null) {
          $path[$max_child] = true;
          $path_edges[profiler_build_parent_child_key($node, $max_child)] = true;
        }
        $node = $max_child;
      } else {
        $node = null;
 ...
 if ($source == "bm" && array_key_exists("main()", $sym_table)) {
  ...
  foreach ($raw_data as $parent_child => $info) {
    ...
    if (isset($sym_table[$parent]) && isset($sym_table[$child]) &&
      ...
      if ($critical_path &&
          isset($path_edges[profiler_build_parent_child_key($parent, $child)])) {
...
function  profiler_render_diff_image($profiler_runs_impl, $run1, $run2,
                                   $type, $threshold, $source) {
  $total1;
  $total2;

  $raw_data1 = $profiler_runs_impl->get_run($run1, $source, $desc_unused);
  $raw_data2 = $profiler_runs_impl->get_run($run2, $source, $desc_unused);

  // init_metrics($raw_data1, null, null);
  $children_table1 = profiler_get_children_table($raw_data1);
  $children_table2 = profiler_get_children_table($raw_data2);
  $symbol_tab1 = profiler_compute_flat_info($raw_data1, $total1);
  $symbol_tab2 = profiler_compute_flat_info($raw_data2, $total2);
  $run_delta = profiler_compute_diff($raw_data1, $raw_data2);
  $script = profiler_generate_dot_script($run_delta, $threshold, $source,
                                       null, null, true,
                                       $symbol_tab1, $symbol_tab2);
  $content = profiler_generate_image_by_dot($script, $type);

  profiler_generate_mime_header($type, strlen($content));
  echo $content;
}
 ...
 * @author cjiang
 */
function profiler_get_content_by_run($profiler_runs_impl, $run_id, $type,
  ...
  if (!$raw_data) {
    profiler_error("Raw data is empty");
    return "";
  }

  $script = profiler_generate_dot_script($raw_data, $threshold, $source,
 ...
 * @author cjiang
 */
function profiler_render_image($profiler_runs_impl, $run_id, $type, $threshold,
                             $func, $source, $critical_path) {

  $content = profiler_get_content_by_run($profiler_runs_impl, $run_id, $type,
                                       $threshold,
                                       $func, $source, $critical_path);
  if (!$content) {
