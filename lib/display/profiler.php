...
//
// profiler: A Hierarchical Profiler for PHP
//
// profiler has two components:
//
//  * This module is the UI/reporting component, used
//    for viewing results of profiler runs from a browser.
//
//  * Data collection component: This is implemented
//    as a PHP extension (profiler).
//
// @author Kannan Muthukkaruppan
//

if (!isset($GLOBALS['PROFILER_LIB_ROOT'])) {
  // by default, the parent directory is profiler lib root
  $GLOBALS['PROFILER_LIB_ROOT'] = realpath(dirname(__FILE__) . '/..');
}

require_once $GLOBALS['PROFILER_LIB_ROOT'].'/utils/lib.php';
require_once $GLOBALS['PROFILER_LIB_ROOT'].'/utils/callgraph_utils.php';
require_once $GLOBALS['PROFILER_LIB_ROOT'].'/utils/runs.php';
 ...
 * a different location that than 'profiler_html' directory the
 * caller must provide the URL path to 'profiler_html' directory
 * so that the correct location of the style sheets/javascript
 * can be specified in the generated HTML.
 *
 */
function profiler_include_js_css($ui_dir_url_path = null) {
  ...
  // style sheets
  echo "<link href='$ui_dir_url_path/css/profiler.css' rel='stylesheet' ".
    " type='text/css' />";
  ...
  echo "<script src='$ui_dir_url_path/jquery/jquery.autocomplete.js'>"
       ."</script>";
  echo "<script src='$ui_dir_url_path/js/profiler_report.js'></script>";
}


/*
 * Formats call counts for profiler reports.
 *
 * Description:
 ...
 *
 */
function profiler_count_format($num) {
  $num = round($num, 3);
  if (round($num) == $num) {
...
function profiler_percent_format($s, $precision = 1) {
 ...
 */
function profiler_render_actions($actions) {
  $out = array();

  if (count($actions)) {
    $out[] = '<ul class="profiler_actions">';
 ...
 * @param raw-str  $rel
 */
function profiler_render_link($content, $href, $class='', $id='', $title='',
                            $target='',
...
$format_cbk = array(
                      "fn" => "",
                      "ct" => "profiler_count_format",
                      "Calls%" => "profiler_percent_format",

                      "wt" => "number_format",
                      "IWall%" => "profiler_percent_format",
                      "excl_wt" => "number_format",
                      "EWall%" => "profiler_percent_format",

                      "ut" => "number_format",
                      "IUser%" => "profiler_percent_format",
                      "excl_ut" => "number_format",
                      "EUser%" => "profiler_percent_format",

                      "st" => "number_format",
                      "ISys%" => "profiler_percent_format",
                      "excl_st" => "number_format",
                      "ESys%" => "profiler_percent_format",

                      "cpu" => "number_format",
                      "ICpu%" => "profiler_percent_format",
                      "excl_cpu" => "number_format",
                      "ECpu%" => "profiler_percent_format",

                      "mu" => "number_format",
                      "IMUse%" => "profiler_percent_format",
                      "excl_mu" => "number_format",
                      "EMUse%" => "profiler_percent_format",

                      "pmu" => "number_format",
                      "IPMUse%" => "profiler_percent_format",
                      "excl_pmu" => "number_format",
                      "EPMUse%" => "profiler_percent_format",

                      "samples" => "number_format",
                      "ISamples%" => "profiler_percent_format",
                      "excl_samples" => "number_format",
                      "ESamples%" => "profiler_percent_format",
                      );
...
function profiler_report ($url_params,
  ...
  if (!empty($rep_symbol)) {
    $run1_data = profiler_trim_run($run1_data, array($rep_symbol));
    if ($diff_mode) {
      $run2_data = profiler_trim_run($run2_data, array($rep_symbol));
    }
  }

  if ($diff_mode) {
    $run_delta = profiler_compute_diff($run1_data, $run2_data);
    $symbol_tab  = profiler_compute_flat_info($run_delta, $totals);
    $symbol_tab1 = profiler_compute_flat_info($run1_data, $totals_1);
    $symbol_tab2 = profiler_compute_flat_info($run2_data, $totals_2);
  } else {
    $symbol_tab = profiler_compute_flat_info($run1_data, $totals);
  }

  $run1_txt = sprintf("<b>Run #%s:</b> %s",
                      $run1, $run1_desc);

  $base_url_params = profiler_array_unset(profiler_array_unset($url_params,
                                                           'symbol'),
                                        'all');
  ...
  if ($diff_mode) {
    $diff_text = "Diff";
    $base_url_params = profiler_array_unset($base_url_params, 'run1');
    $base_url_params = profiler_array_unset($base_url_params, 'run2');
    $run1_link = profiler_render_link('View Run #' . $run1,
                           "$base_path/?" .
                           http_build_query(profiler_array_set($base_url_params,
                                                      'run',
                                                      $run1)));
    $run2_txt = sprintf("<b>Run #%s:</b> %s",
                        $run2, $run2_desc);

    $run2_link = profiler_render_link('View Run #' . $run2,
                                    "$base_path/?" .
                        http_build_query(profiler_array_set($base_url_params,
                                                          'run',
                                                          $run2)));
  ...
  $links = array();
  $links [] =  profiler_render_link("View Top Level $diff_text Report",
  ...
  if ($diff_mode) {
    ...
    $links [] = $run1_link;
    $links [] = $run2_link;
    $links [] = profiler_render_link('Invert ' . $diff_text . ' Report',
                           "$base_path/?".
  ...
  if (!empty($rep_symbol)) {
    if (!isset($symbol_tab[$rep_symbol])) {
      echo "<hr>Symbol <b>$rep_symbol</b> not found in profiler run</b><hr>";
      return;
    }
...
function print_td_pct($number, $denom, $bold=false, $attributes=null) {
  ...
  if ($denom == 0) {
    ...
    $pct = profiler_percent_format($number / abs($denom));
  }
...
function print_function_info($url_params, $info, $sort, $run1, $run2) {
  ...
  $href = "$base_path/?" .
           http_build_query(profiler_array_set($url_params,
                                             'symbol', $info["fn"]));

  print('<td>');
  print(profiler_render_link($info["fn"], $href));
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
  ...
  if (!$limit) {              // no limit
    ...
    $display_link = profiler_render_link(" [ <b class=bubble>display all </b>]",
                                       "$base_path/?" .
                                       http_build_query(profiler_array_set($url_params,
                                                                         'all', 1)));
  }
  ...
  foreach ($stats as $stat) {
    ...
    if (array_key_exists($stat, $sortable_columns)) {
      $href = "$base_path/?"
              . http_build_query(profiler_array_set($url_params, 'sort', $stat));
      $header = profiler_render_link($desc, $href);
    } else {
      $header = $desc;
    }
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
  ...
  if ($diff_mode) {
    ...
    print('<tr bgcolor="#bdc7d8" align=right>');
    print("<th></th>");
    print("<th $vwbar>" . profiler_render_link("Run #$run1", $href1) . "</th>");
    print("<th $vwbar>" . profiler_render_link("Run #$run2", $href2) . "</th>");
  ...
  print("<center><br><h3>" .
        profiler_render_link($callgraph_report_title,
...
function print_pc_array($url_params, $results, $base_ct, $base_info, $parent,
  ...
  foreach ($results as $info) {
    ...
    print("<td>" . profiler_render_link($info["fn"], $href));
    print_source_link($info);
    print("</td>");
...
function print_source_link($info) {
  if (strncmp($info['fn'], 'run_init', 8) && $info['fn'] !== 'main()') {
	if (defined('PROFILER_SYMBOL_LOOKUP_URL')) {
      $link = profiler_render_link(
        'source',
        PROFILER_SYMBOL_LOOKUP_URL . '?symbol='.rawurlencode($info["fn"]));
      print(' ('.$link.')');
    }
  }
}
...
function symbol_report($url_params,
  ...
  if ($diff_mode) {
    ...
    $base_url_params = profiler_array_unset(profiler_array_unset($url_params,
                                          ...
                                          'run2');
    $href1 = "$base_path?"
      . http_build_query(profiler_array_set($base_url_params, 'run', $run1));
    $href2 = "$base_path?"
      . http_build_query(profiler_array_set($base_url_params, 'run', $run2));
  ...
  $callgraph_href = "$base_path/callgraph.php?"
    . http_build_query(profiler_array_set($url_params, 'func', $rep_symbol));
  ...
  // Related javascript code is in: profiler_report.js
  print("\n");
...
function profiler_single_run_report ($url_params,
  ...
  init_metrics($profiler_data, $rep_symbol, $sort, false);
...
function profiler_diff_report($url_params,
  ...
  init_metrics($profiler_data2, $rep_symbol, $sort, true);
...
/**
 * Generate a profiler Display View given the various URL parameters
 ...
 *
 */
function displayProfilerReport($profiler_runs_impl, $url_params, $source,
  ...
  if ($run) {                              // specific run to display?
    ...
    if (count($runs_array) == 1) {
      $profiler_data = $profiler_runs_impl->get_run($runs_array[0],
                                                $source,
                                                $description);
    } else {
      if (!empty($wts)) {
      ...
      $data = profiler_aggregate_runs($profiler_runs_impl,
                                    $runs_array, $wts_array, $source, false);
      $profiler_data = $data['raw'];
    ...
    $profiler_data1 = $profiler_runs_impl->get_run($run1, $source, $description1);
    $profiler_data2 = $profiler_runs_impl->get_run($run2, $source, $description2);
    ...
    echo "No profiler runs specified in the URL.";
    if (method_exists($profiler_runs_impl, 'list_runs')) {
      $profiler_runs_impl->list_runs();
    }
  }
}
